...
import com.trs.netInsight.support.ckm.impl.CkmServiceImpl;
 ...
 */
@Service("hybase8SearchService")
@Slf4j
public class Hybase8SearchImpl implements FullTextSearch {
	...
	public <T extends IDocument> List<T> ftsQuery(QueryBuilder query, Class<T> resultClass, boolean isSimilar,
			boolean irSimflag,boolean irSimflagAll,String type) throws TRSSearchException {
		return ftsQuery(query.getKeyRedis(), query.asTRSL(), query.getOrderBy(), query.getPageSize(), query.getPageNo(),
				resultClass, isSimilar,irSimflag,irSimflagAll,query.getDataDateAndDataSources() == null?true:false, query.isServer(),type);
	}

	@Override
	public <T extends IDocument> List<T> ftsQuery(QueryBuilder query, Class<T> resultClass, boolean isSimilar,
			boolean irSimflag,boolean irSimflagAll, boolean isDataDateAndDataSources,String type) throws TRSSearchException {
		return ftsQuery(query.getKeyRedis(), query.asTRSL(), query.getOrderBy(), query.getPageSize(), query.getPageNo(),
				resultClass, isSimilar, irSimflag,irSimflagAll,isDataDateAndDataSources, query.isServer(),type);
	}

	@Override
	public <T extends IDocument> List<T> ftsQueryForExport(QueryBuilder query, Class<T> resultClass, boolean isSimilar,
			boolean irSimflag,boolean irSimflagAll,String type) throws TRSSearchException {
		return ftsQueryForExport(query.getKeyRedis(), query.asTRSL(), query.getOrderBy(), query.getPageSize(),
	 ...
	 * @Return : List<T>
	 */
	private <T extends IDocument> List<T> ftsQuery(String keyRedis, String trsl, String orderBy, long pageSize,
			long pageNo, Class<T> resultClass, boolean isSimilar, boolean irSimflag,boolean irSimflagAll, boolean isDataDateAndDataSources,
			boolean server,String type) throws TRSSearchException {
		queryCount();
		// 判断是否排重
		...
		trsl = commonMonthd(trsl, isSimilar, irSimflag, irSimflagAll,isDataDateAndDataSources, resultClass);
		...
		try {
			...
			String search = extractByTrsl(trsl, isDataDateAndDataSources,type);
			if (server) {
				trsl = changgeServer(searchParams, trsl);
			}
			log.warn(trsl);
		//	System.err.println(trsl);
			if (search != null) {
			...
			queryTime(startHybase, endHybase, trsl, queryTime, connectTime, db, connection.getURL(), server);
	 ...
	 * @throws TRSSearchException
	 */
	private <T extends IDocument> List<T> ftsQueryForExport(String keyRedis, String trsl, String orderBy, long pageSize,
	...
	public long ftsCount(QueryBuilder query, boolean isSimilar, boolean irSimflag,boolean irSimflagAll,String type) {
		...
		try {
			...
			// 此处导致同一条微博信息在热点列表和普通列表相似文章数不一致，，，此处限制只查原发也不合理
			// if (databases.contains(Const.WEIBO)) {
			// query.filterByTRSL("IR_RETWEETED_MID:(0 OR \"\")");
			// }
			// log.info(query.asTRSL());
			...
			int connectTime = (int) (endConnect - startConnect);
			if (query.isServer()) {
				similarTrsl = changgeServer(param, similarTrsl);
			}
			long startHybase = new Date().getTime();
	...
	@Override
	public <T extends IDocument> long ftsCount(QueryBuilder query, Class<T> resultClass, boolean isSimilar,
			boolean irSimflag,boolean irSimflagAll,String type) {
		queryCount();
		SearchParams param = new SearchParams();
		param.setTimeOut(60);
		String search = extractUrlTimeByBuilder(query, true,type);
...
//			}
			...
			log.error("Fail to count by hybase ...:" + e);
			return 0;
		} finally {
			HybaseFactory.clean();
		}
	...
	@Override
	public <T extends IDocument> long ftsCountNoRtt(QueryBuilder query, Class<T> resultClass, boolean isSimilar,
			boolean irSimflag,boolean irSimflagAll,String type) {
		queryCount();
		SearchParams param = new SearchParams();
		param.setTimeOut(60);
		...
		try {
			String databases = FtsParser.getDatabases(resultClass);
			String db = addHybaseInsert(databases);
			String similarTrsl = commonMonthd(query.asTRSL(), isSimilar, irSimflag,irSimflagAll, true, resultClass);
			log.info("ftsCountNoRtt:" + similarTrsl);
			long startConnect = new Date().getTime();
			TRSConnection client = HybaseFactory.getClient();
			long endConnect = new Date().getTime();
			int connectTime = (int) (endConnect - startConnect);
	 ...
	 */
	@Override
	public GroupResult categoryQuery(boolean server, String trsl, boolean isSimilar, boolean irSimflag,boolean irSimflagAll,
		...
		try {
			...
			// RedisUtil.setString(redisKey, trsl, 10, TimeUnit.MINUTES);
			// }
	 ...
	 */
	@Override
	public GroupResult categoryQueryNoSort(boolean server, String trsl, boolean isSimilar, boolean irSimflag,boolean irSimflagAll,
			String groupField, int limit,String type, String... indices) throws TRSSearchException {
		queryCount();
		String db = addHybaseInsert(String.join(";", indices));
		trsl = commonMonthd(trsl, isSimilar, irSimflag,irSimflagAll, true, indices);
		...
		try {
			...
			// 系统日志记录
			systemLogRecord(trsl);
			// TRSResultSet result = connection.categoryQuery(db, trsl, "",
			// groupField, limit);
			long endHybase = new Date().getTime();
			int queryTime = (int) (endHybase - startHybase);
			categoryTime(startHybase, endHybase, trsl, queryTime, connectTime, db, groupField, connection.getURL(),
	...
	public void queryTime(long startHybase, long endHybase, String trsl, int time, int connectTime, String db,
			String linked, boolean server) {
		long id = Thread.currentThread().getId();
		// String trslk = RedisUtil.PER_KEY+id;
		// if(server){
		// trslk = trslk+RedisUtil.SUFFIX_KEY;
		// }
		// if(trsl.contains(FtsFieldConst.WEIGHT)){
		// trslk = trslk+FtsFieldConst.WEIGHT;
		// }
		// //存入完整表达式
		// Set<String> keys = RedisUtil.getKeys(trslk);
		// if(keys!=null & keys.size()>0){
		// RedisUtil.setString(trslk, trsl);
		// }
		LogPrintUtil loginpool = RedisUtil.getLog(Thread.currentThread().getId());
		if (null != loginpool) {
	...
	public void categoryTime(long startHybase, long endHybase, String trsl, int time, int connectTime, String db,
			String field, String linked, boolean server) {
		long id = Thread.currentThread().getId();
		// String trslk = RedisUtil.PER_KEY+id;
		// if(server){
		// trslk = trslk+RedisUtil.SUFFIX_KEY;
		...
		// }
		// //存入完整表达式
		// Set<String> keys = RedisUtil.getKeys(RedisUtil.PER_KEY+id);
		// if(keys!=null & keys.size()>0){
		// //取一个就好
		// String next = keys.iterator().next();
		// RedisUtil.setString(next, trsl);
		// }
	...
	@Override
	public Iterator groupCount(QueryBuilder queryBuilder, String groupBy, boolean isSimilar, boolean irSimflag,boolean irSimflagAll,String type)
			throws TRSSearchException {
		TRSConnection connection = null;
		String similarTrsl = null;
		try {
			String database = queryBuilder.getDatabase();
			log.info("4546464:" + queryBuilder.asTRSL());
			connection = HybaseFactory.getClient();
			SearchParams searchParams = new SearchParams();
			searchParams.setTimeOut(60);
			String search = extractUrlTimeByBuilder(queryBuilder, true,type);
			String db = addHybaseInsert(database);
			similarTrsl = commonMonthd(queryBuilder.asTRSL(), isSimilar, irSimflag,irSimflagAll, true, database);
			if (queryBuilder.isServer()) {
				similarTrsl = changgeServer(searchParams, similarTrsl);
			}
			if (search != null) {
				searchParams.setProperty("search.range.filter", search);
			}
			TRSResultSet result = connection.categoryQuery(db, similarTrsl, null, groupBy,
					Long.MAX_VALUE, searchParams);
			// 系统日志记录
			systemLogRecord(similarTrsl);

			// TRSResultSet result =
			// connection.categoryQuery(isChuantong(database),
			// commonMonthd(queryBuilder.asTRSL(), isSimilar,irSimflag,
			// database),
			// null, groupBy, Long.MAX_VALUE);
			Map<String, Long> categoryMap = result.getCategoryMap();
			Iterator<Map.Entry<String, Long>> iter = categoryMap.entrySet().iterator();
			return iter;
		} catch (Exception e) {
...
//			if (StringUtils.isNotBlank(trsl)) {
//				trsl = trsl + " AND " + FtsFieldConst.FIELD_IR_SIMFLAGALL + ":(0 OR \"\")";
//			} else {
//				trsl = FtsFieldConst.FIELD_IR_SIMFLAGALL + ":(0 OR \"\")";
//			}
		}
		// 该配置默认清除历史配置
		if (irSimflagOpen) {
	 ...
	 * @return
	 */
	private String isChuantong(String indices) {
		String[] split = indices.split("[;|,]");
		List<String> list = Arrays.asList(split);
		if (list.contains(Const.HYBASE_NI_INDEX)) {
...
private String isArticleDelete(String startTrsl,String... indices){
	...
	private String isArticleDelete(String... indices) {
		...
		// 原来没考虑到传两个数据源进来的时，只查了其中一个数据源的删除文章
//		if (list.contains(Const.HYBASE_NI)) {
//			trsl = FtsFieldConst.FIELD_SID;
//			groupName = ArticleDeleteGroupName.chuantong;
//		} else if (list.contains(Const.WEIBO)) {
//			trsl = FtsFieldConst.FIELD_MID;
//			groupName = ArticleDeleteGroupName.weibo;
//		} else if (list.contains(Const.WECHAT)) {
//			trsl = FtsFieldConst.FIELD_SID;
//			isGroupNameWeixin = true;
//			groupName = ArticleDeleteGroupName.weixin;
//		}
//		resultTrsl = articleDeleteTrsl(trsl, isGroupNameWeixin, groupName);
		return resultTrsl;
	}
...
//						if (isGroupNameWeixin) {
//							trsl = " NOT " + FtsFieldConst.FIELD_HKEY + ":(" + buffer.toString() + ")";
//						}
						//判断大于16kb,删除排重去掉
...
//						}
	...
	public PagedList<FtsDocumentCommonVO> pageListCommonForExport(QueryCommonBuilder query, boolean isSimilar,
		...
		try {
			...
			queryTime(timeStart, timeEnd, trsl, queryTime, connectTime, db, connection.getURL(), query.isServer());
			if (StringUtil.isNotEmpty(query.getKeyRedis())) {
	...
	private String changgeServer(SearchParams searchParams, String trsl) {
		trsl = trsl.replace(":", "=").replace("AND", "*").replace("OR", "+").replace("NOT", "-").replace("\"", "\'");
		if (trsl.contains("http")) {
			int index = trsl.indexOf("http");
			char charAt = trsl.charAt(index + 4);
			if ("=".equals(String.valueOf(charAt))) {
				StringBuilder sb = new StringBuilder(trsl);
				sb.replace(index + 4, index + 5, ":");
				trsl = sb.toString();
			}
		}
		searchParams.setProperty("search.syntax.name", "trs");
		return trsl;
	}
