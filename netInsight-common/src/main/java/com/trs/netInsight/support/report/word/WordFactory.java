...
import com.trs.netInsight.support.report.word.model.Status;
import com.trs.netInsight.support.report.word.model.WordData;
import com.trs.netInsight.support.report.word.process.DataFillAnalyzer;
import com.trs.netInsight.support.report.word.process.MarkAnalyzer;
import com.trs.netInsight.support.report.word.process.StyleAnalyzer;
 ...
 * xmlbeans-2.6.0.jar
 * 
 * @author slzs 2014-4-9 下午3:03:40 each engineer has a duty to keep the code
 *         elegant
 */
@Slf4j
public class WordFactory {
	...
	/**
	 * @author slzs 2017年5月9日 上午9:16:29
	 * @param template
	 * @throws IOException
	 */
	private XWPFDocument loadTemplate(Object template) {
		...
		try {
			...
			if (template instanceof String) {
				...
				log.error("开始执行POIXMLDocument.openPackage(template.toString())");
				OPCPackage openPackage = POIXMLDocument.openPackage(template.toString());
				log.error("POIXMLDocument.openPackage(template.toString()) 执行完毕");
				log.error("openPackage:" + openPackage);
				document = new XWPFDocument(openPackage);
