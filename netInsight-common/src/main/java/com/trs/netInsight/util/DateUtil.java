...
public class DateUtil {
	...
	public static List<String> getBetweenDateHourString(String beginDate, String endDate, String dateFormat) {
		...
		for (GregorianCalendar g : v) {
			for (int i = 0; i < 24; i++) {
				if (i >= 10) {
					list.add(DateUtil.format2String(g.getTime(), dateFormat) + i + "0000");
				} else {
					list.add(DateUtil.format2String(g.getTime(), dateFormat) + "0" + i + "0000");
				}
			}
		}
		return list;
	...
	public static String[] formatTimeRange(String time) throws OperationException {
		...
		//含有 至今时，具体查询时间范围，需和机构保持一致  //貌似不用加这个，hybase方式有个时间filter控制
		/*if (time.indexOf("至今") != -1) {
			return upToNowToOrgTime(time);
		}
		*/
...
//			timeArray[1] = new SimpleDateFormat(yyyyMMddHHmmss).format(new Date());
			timeArray[1] = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date());
			switch (timeFlag) {
			case 'h':
...
//		return new SimpleDateFormat(yyyyMMddHHmmss).format(cal.getTime());
	...
	public static  String[] getTimeInterval(Date date, String format, int day) {
		...
		String endTime = null;
		int now = com.trs.netInsight.support.fts.util.DateUtil.rangBetweenNow(date);
		if (now == 0) {
			beginTime = com.trs.netInsight.support.fts.util.DateUtil.formatDateAfter(date, format, -day * 2);
			endTime = com.trs.netInsight.support.fts.util.DateUtil.formatCurrentTime(format);
		} else {
			beginTime = com.trs.netInsight.support.fts.util.DateUtil.formatDateAfter(date, format, -day * 2 - now);
			endTime = com.trs.netInsight.support.fts.util.DateUtil.formatDateAfter(date, format,  -now);
		}
	 ...
	 * @return
	 */
	public static int differenceOfCurrentDate(Date date){
		long nowTime = new Date().getTime();
	...
	/**
	 * 计算指定时间  从开始到至今与三个月的关系，将至今换为具体时间
	 *
	 * @param timeRange
	 * @return
	 */
	public static String getStartToThreeMonth(String timeRange) throws OperationException {
		String[] timeArray = new String[2];
		if (timeRange.indexOf("至今") != -1) {
			timeRange = timeRange.replace("至今", formatCurrentTime("yyyy-MM-dd HH:mm:ss"));
			timeArray = timeRange.split(";");
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
			//字符串转换成LocalDateTime类型
			LocalDateTime startTime = LocalDateTime.parse(timeArray[0], dtf);
			LocalDateTime currentTime = LocalDateTime.parse(timeArray[timeArray.length - 1], dtf);
			LocalDateTime endTime = startTime.plusDays(90);
			if (endTime.isBefore(currentTime)) { //多于三个月
				timeArray[0] = startTime.format(dtf);
				timeArray[timeArray.length - 1] = endTime.format(dtf);
				timeRange = String.join(";", timeArray);
			}else{
				timeArray[0] = startTime.format(dtf);
				timeArray[timeArray.length - 1] = "至今";
				timeRange = String.join(";", timeArray);
			}
		}
		return timeRange;
	 ...
	 * @return
	 */
//	public static String[] upToNowToOrgTime(String timeRange) throws OperationException {
//		String[] timeArray = new String[2];
//			//含有至今
//			timeRange = timeRange.replace("至今", formatCurrentTime("yyyy-MM-dd HH:mm:ss"));
...
//				String[] strings = split[0].split("\\s+");
//				if (ObjectUtil.isNotEmpty(strings)){
//					start = com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.stringToDate(strings[0], com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.yyyyMMdd3);
//				}
//			}
...
//
//				// 机构开始时间
//				Date beginDate = com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.dateAfter(endDate, -dataDate);
//				if (beginDate != null){
...
//			}
//			if (null != start){
//				timeArray[0] = com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.date2String(start, com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.yyyyMMddHHmmssSSS);
//				timeArray[1] = com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.date2String(endDate, com.trs.netInsight.com.trs.netInsight.support.fts.com.trs.netInsight.util.DateUtil.yyyyMMddHHmmssSSS);
//				return timeArray;
//			}
//
//		return null;
//	}
	 ...
	 * @return
	 */
	public static String timeDifferenceDays(String  expireAt) {
		long nowTime = new Date().getTime();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		try {
			Date parse = sdf.parse(expireAt);
			long time = parse.getTime();
			long result = time - nowTime;
			if (result < 0){
				return "过期";
			}
			int days = (int) (result / (1000 * 60 * 60 * 24));
			//不足一天按 一天计算 （2019-11-27）
			if ((result % (1000 * 60 * 60 * 24)) > 0){
				days += 1;
			}
			String valueOf = String.valueOf(days);
			if (days < 1 && valueOf.length() > 3){
	 ...
	 * 比较两个string类时间的大小
	 * @param time        被比较的时间
	 * @param currentTime  当前需要比较的时间
	 * @return
	 */
	public static boolean isExpire(String time,String currentTime){
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

		try {
			if (sdf.parse(currentTime).getTime() <= sdf.parse(time).getTime()){
				return true;
			}
		} catch (ParseException e) {
			return false;
